...
function initScrollAnimations() {
        ...
        globalRevealObserver = new IntersectionObserver((entries, obs) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    // Gỡ bỏ observer ngay khi gọi để giải phóng bộ nhớ
                    obs.unobserve(entry.target); 
                }
            });
        }, { 
            // Tải trước thẻ cách xa 600px dưới mép màn hình. 
            // Điều này giúp loại bỏ HOÀN TOÀN việc chờ khi vuốt
    ...
    newCards.forEach((card) => {
        // Hoàn toàn không tính toán JS Delay. Rất nhẹ.
